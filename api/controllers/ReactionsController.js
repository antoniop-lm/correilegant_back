...
module.exports = {
    ...
    */
    ...
    if (data) {
    	var tweet = Tweets.findOne({id: data.tweet}).exec(function (err, record){
    		if(err) throw err;
    		console.log(record);
    		return record;
    	} );
    	var user = Users.findOne({username: data.user}).exec(function (err, record){
    		if(err) throw err;
    		console.log(record);
    		return record;
    	} );
    	...
    	if (user && tweet) {
	    	try {
	    		Reactions.create(data).exec(function createCB(err, created){
	    			if(err) throw err;
	    			var aux = "";
